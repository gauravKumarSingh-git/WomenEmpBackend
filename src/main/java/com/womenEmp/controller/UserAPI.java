...
public class UserAPI {
	...
	public ResponseEntity<UserDTO> login(@PathVariable 
			@Pattern(regexp = "[a-zA-Z0-9]+( [A-Za-z0-9]+)*", message = "{user.username.invalid}")
			String username, 
			@PathVariable 
	...
	@DeleteMapping("/UserLogin/{userId}")
	public ResponseEntity<String> deleteUser(@PathVariable Integer userId) throws WomenEmpException{
		String ret = userService.deleteUser(userId);
		LOGGER.info(environment.getProperty("UserAPI.DELETE_SUCCESS"));
		return new ResponseEntity<>(ret, HttpStatus.OK);
	}
}
